...
def fit_cylinder_least_squares(points):
    ...
    center = [  3.48940202,   3.42034294, -38.60175442] #np.mean(points, axis=0)
    _, _, vh = np.linalg.svd(points - center)
    axis_guess = np.array([2, -1, 0],dtype=float)  # First principal component as initial axis guess
    radius_guess = 25.0 #np.mean(np.linalg.norm(points - center, axis=1)) / 2
    
    cylinder_params = list(center) + list(axis_guess) + [radius_guess]
    cylinder = generate_fitted_cylinder(cylinder_params, ecw)
    mt.savemesh(cylinder,directory+'/Templates/cylinder.obj')
...
head = '/Templates/Pig7_Pre_R_Head'
condyles = '/Templates/Pig7_Pre_R_Condyles'
neck = '/Templates/Pig7_Pre_R_Neck'
shaft = '/Templates/Pig7_Pre_R_Shaft'
baseside = 'R'
...
mt.savemesh(sphere,directory+'/Templates/head.obj')

# Fit a cylinder to the condyles
Bb = tri.load_mesh(directory+'/'+condyles+'.obj')
ecw = 53.0 #Epicondyle width measured manually.
